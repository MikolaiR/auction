...
@section('title', __('Admin Support Details'))
@section('content')
...
<div class="main-content app-content mt-0">
    <div class="side-app">
        ...
        <div class="main-container container-fluid">
            @include('layouts.breadcrumb', ['admin' => true, 'pageTitle' => __('Support Details'), 'hasBack' => true, 'backTitle' => __('All Support'), 'backUrl' => route('admin.support.index')])
            <div class="row">
                <div class="col-12 col-sm-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title mb-0">{{ __('Support Details') }}</h3>
                        </div>
                        <div class="row">
                            <div class="col-xl-12">
                                <div class="card">
                                    <div class="card-body" id="email-body">
                                        <div class="email-media">
                                            <div class="mt-0 d-sm-flex">
                                                <img class="me-2 rounded-circle avatar avatar-lg" src="{{ get_random_avatar() }}" alt="{{ __('avatar') }}">
                                                <div class="media-body pt-0">
                                                        ...
                                                        <div class="me-3">
                                                            ...
                                                            <div class="dropdown-menu dropdown-menu-end">
                                                                <form action="{{route('admin.support.destroy', $support->id)}}" method="POST">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button class="dropdown-item" type="submit"><i class="fa-regular fa-trash me-2"></i>{{ __('Delete') }}</button>
                                                                </form>
                                                                <a class="dropdown-item" onclick="printSupport()" href="javascript:void(0)"><i class="fa-regular fa-print me-2"></i>{{ __('Print') }}</a>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <small class="mb-0">{{ __('to') }} {{config('app.name')}} {{ __('Support Team') }} ({{$support->admin->email ?? __('No Admin')}})</small>

                                                </div>
                                            </div>
                                        </div>
                                        <div class="eamil-body mt-5">
                                            {!! $support->message ?? __('No message') !!}</>
                                        </div>
                                    </div>
                                    <div class="card-footer">
                                        @if($support->response)
                                        <div class="fw-bold mt-1">{{ __('Response from') }} {{config('app.name')}} {{ __('Support Team') }}</div>
                                        <div class="eamil-body mt-5">
                                            {!! $support->response !!}
                                        </div>
                                        @else
                                        <form action="{{route('admin.support.update', $support->id)}}" method="POST">
                                            @method('PUT')
                                            @csrf
                                            <x-text-area-field name="response" label="{{ __('Reply') }}" placeholder="{{ __('Enter Reply') }}" :admin="true" />
                                            <div class="row">
                                                <label class="col-md-3 form-label mb-4">{{ __('Status *') }} :</label>
                                                <div class="col-md-9">
                                                    <select class="form-control select2" name="status" id="status" data-bs-placeholder="{{ __('Select Status') }}">
                                            ...
                                            <div class="d-sm-flex pt-4">
                                                <div class="btn-list ms-auto my-auto">
                                                    ...
                                                    <button type="submit" class="btn btn-primary btn-space mb-0">{{ __('Update') }}</button>
                                                </div>
                                            </div>
                                        </form>
                                        @endif
